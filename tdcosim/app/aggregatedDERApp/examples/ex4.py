...
if __name__=='__main__':
	try:
		...
		dataDir=os.path.join(baseDir,'data','68bus')
		rawFilePath=os.path.join(dataDir,'case68.raw')
		dyrFilePath=os.path.join(dataDir,'case68.dyr')
		...
		thisApp.load_case(rawFilePath,dyrFilePath,adjustSystemOperatingPoint=True,\
		reductionPercent=0.2)

		# add cmld
		thisApp.convert_load_to_cmld(cleanup=True)
		...
		thisApp.add_dera_to_case({'1547_2003':[2,3]},rating=rating,solarPercentage=0.2)
		...
		df.to_pickle(os.path.join(baseDir,'data','out','result_ex4_cmld.p'))
	except:
