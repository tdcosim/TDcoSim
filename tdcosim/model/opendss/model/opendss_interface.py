...
class OpenDSSInterface:
    ...
    def getVoltage(self,vtype='actual'):
        """Needs to be called after solve. Gathers Voltage (complex) of all buses into a,b,c phases and populates a dictionary and returns it."""
        try:
            Voltage={}
            entryMap={}
            entryMap['1']='a'
            ...
            for n in range(0,self.Circuit.NumBuses):
                ...
                for entry in self.Circuit.Buses(n).Nodes:
                    Voltage[self.Circuit.Buses(n).Name][entryMap[str(int(entry))]] = V[count]+1j * V[count+1]
                    count+=2
