		...
		try:
			...
			if 'myconfig' in OpenDSSData.config and 'DERParameters' in OpenDSSData.config['myconfig']:
				...
				if 'nodenumber' in OpenDSSData.config['myconfig']:
					DERLocation = str(os.getpid()) + '-' + 'bus_'+\
					str(OpenDSSData.config['myconfig']['nodenumber']) +'-'+ 'node_' + nodeid
				else:
			...
			OpenDSSData.logger.debug('Creating DER instance of {} model for {} node.'.format(DERModelType,DERLocation))
...
#===================================================================================================
	def prerun(self,gridVoltagePhaseA,gridVoltagePhaseB,gridVoltagePhaseC,dt=1/120.0):
		...
		try:
			...
			OpenDSSData.log(msg="Failed prerun in the pvder model")
...
#===================================================================================================
	def postrun(self,sol,t,KVAbase=50):
		...
		try:
			...
			OpenDSSData.log(msg="Failed postrun in the pvder model")
...
#===================================================================================================
	def _run(self,gridVoltagePhaseA,gridVoltagePhaseB,gridVoltagePhaseC,KVAbase=50):
		...
		try:
			...
			self.PV_model.gridVoltagePhaseC=gridVoltagePhaseC*math.sqrt(2)/Grid.Vbase
			self.sim.t = t
			sol,info,ConvergeFlag = self.sim.call_ODE_solver(self.sim.ODE_model,
			self.sim.jac_ODE_model,self.lastSol,t)

			if self.sim.COLLECT_SOLUTION:
				self.sim.collect_solution(sol,t)
			#sol,info=odeint(odeFunc,y0,t,Dfun=jac_odeFunc,full_output=1,printmessg=True,
			#hmax = 1/120.,mxstep=50,atol=1e-4,rtol=1e-4)
			#sol,info=odeint(odeFunc,y0,t,full_output=1,printmessg=True,hmax = 1/120.,
			#mxstep=50,atol=1e-4,rtol=1e-4)
			...
			OpenDSSData.log(msg="{}:ODE solver failed between {:.4f} s and {:.4f} s!".format(
...
#===================================================================================================
	def _getS(self):
		try:
			...
			OpenDSSData.log(msg="Failed getS in the pvder model")
