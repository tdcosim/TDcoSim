...
def generateReport(GlobalData,fname='report.xlsx',sim='dynamic'):
	...
	try:
		fname=os.path.join(GlobalData.config['outputConfig']['outputDir'],fname)		
		...
		if sim == 'dynamic':
			...
			row=1
			t_all=sorted(data['TNet']['Dynamic'])
			for t in t_all:
		...
		t=sorted(GlobalData.data['monitorData'])
		colMap={}; colMap['time']=0
		for node in GlobalData.data['monitorData'][t[0]]:
			ws[node].write(0,0,'time')
			count=1
			items=sorted(GlobalData.data['monitorData'][t[0]][node])
			for item in items:
		...
		GlobalData.log(msg="Failed generate report")
		GlobalData.log(msg="Unexpected error:" + sys.exc_info()[2])
